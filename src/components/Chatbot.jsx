...
export default function Chatbot() {
  ...
  return (
    <div className="fixed bottom-20 right-4 md:bottom-8 md:right-8 z-50 flex flex-col items-end">
      {isChatOpen && (
        <div
          className="bg-white border text-sm border-slate-200 rounded-lg shadow-xl w-80 mb-4 flex flex-col"
          style={{ height: '400px' }}
        >
          ...
          <div className="flex-grow p-3 overflow-y-auto bg-slate-50 flex flex-col gap-3">
            {chatMessages.length === 0 && (
              <div className="bg-white border border-slate-200 p-2 rounded-lg max-w-[85%] self-start text-slate-700">
            ...
            {chatMessages.map((m, i) => (
              <div
                ...
                className={
                  m.sender === 'user'
                    ? 'bg-slate-900 text-white p-2 rounded-lg max-w-[85%] self-end text-sm break-words'
                    : 'bg-white border border-slate-200 p-2 rounded-lg max-w-[85%] self-start text-slate-700 text-sm break-words'
                }
              >
                {m.text}
              </div>
            ...
            {showPredefinedBtns && chatMessages.length === 0 && (
              <div className="flex flex-wrap gap-2 mt-1">
                <button
                  ...
                  className="bg-blue-50 text-blue-600 text-xs px-2 py-1 border border-blue-200 rounded hover:bg-blue-100 mb-1"
                >
                  Savings Info
                </button>
                <button
          ...
          <div className="p-3 border-t border-slate-200 bg-white rounded-b-lg flex gap-2">
            <input
              type="text"
              value={input}
            ...
            />
            <button onClick={handleSend} className="bg-slate-900 text-white px-3 py-2 rounded-lg">
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
